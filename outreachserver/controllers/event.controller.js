...
module.exports.addEvent = (req, res, next) => {
    ...
    event.save((err, doc) => {
        if (!err) {
            ...
            // console.log({event});
            console.log('************************');
            // mailer.prepAndSendEmail('events', event); 
            mailCtrl.toSendEmail('events', event);
        }
        else {
...
module.exports.deleteEventWithRegUsers = (req, res) => {
    if (!ObjectId.isValid(req.params.id))
        return res.status(400).send('No Event record found with given id: ' + req.params.id);
    else {
        Event.findByIdAndRemove(req.params.id, (err, eve) => {
            if (eve) {
                var regUsers = eve.regUsers;
                for (const regId of regUsers) {
                    Volreg.findByIdAndRemove(regId, (err, vReg) => {
                        if (!err)
                            console.log('Deleted: ' + JSON.stringify(vReg)); //return res.send(event);
                        else
                ...
                console.log('Something went wrong. Please check with system Administrator')
            }
...
module.exports.addBulkEvents = (req, res, next) => {
    ...
    data.forEach(e => {
        ...
        var tProjectId = projectCtrl.asyncFindAndIUProject()
...
module.exports.asyncAddBulkEvents = async (req, res, next) => {
    var bulkEvents = Event.collection.initializeUnorderedBulkOp();
    var data = [] = req.body;
